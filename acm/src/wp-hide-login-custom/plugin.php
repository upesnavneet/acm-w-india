...
// define('WPS_HIDE_LOGIN_URL', '/var/www/html/wp-content/themes/acm/wp-hide-login-custom/');
// define('WPS_HIDE_LOGIN_DIR', '/var/www/html/wp-content/themes/acm/wp-hide-login-custom/plugin.php');

define( 'WPS_HIDE_LOGIN_BASENAME', 'wps-hide-login' );
...
class Plugin {
	...
	protected function init() {
		...
		if ( is_multisite() && true ) {
			...
			add_filter(
				'network_admin_plugin_action_links_' . WPS_HIDE_LOGIN_BASENAME,
				array(
					$this,
					'plugin_action_links',
				)
		...
		add_action( 'after_setup_theme', array( $this, 'plugins_loaded' ), 9999 );
	...
	private function new_login_slug( $blog_id = '' ) {
		if ( $blog_id ) {
			...
			if ( $slug = get_option( 'whl_page' ) ) {
				return $slug;
			} elseif ( ( is_multisite() && true && ( $slug = get_site_option( 'whl_page', 'acm-management' ) ) ) ) {
				return $slug;
			} elseif ( $slug = 'acm-management' ) {
				return $slug;
			}
		}
	}
	...
	private function new_redirect_slug() {
		if ( $slug = get_option( 'whl_redirect_admin' ) ) {
			return $slug;
		} elseif ( ( is_multisite() && true && ( $slug = get_site_option( 'whl_redirect_admin', '404' ) ) ) ) {
			return $slug;
		} elseif ( $slug = '404' ) {
			return $slug;
		}
